...
const ScenarioBuilder = () => {
  ...
  console.log('🏛️ Scenario Builder - Using Organization ID:', organizationId);
  ...
  const loadScenarios = async () => {
    try {
      console.log('📥 Loading scenarios for org:', organizationId);
  ...
  // ✅ FIX: Accept scenarios array as parameter to avoid stale state
  const saveScenarios = async (scenariosToSave = scenarios) => {
    setSaveStatus('saving');
    try {
      console.log('💾 Saving scenarios:', scenariosToSave.length);
      ...
      if (dbConfig) {
        ...
        console.error('❌ No bot config found to update');
        setSaveStatus('error');
      }
    } catch (error) {
  ...
  // ✅ FIX: Pass updated scenarios directly to saveScenarios
  const saveScenario = () => {
    ...
    console.log('💾 Saving scenario:', newScenario.name);
    setScenarios(updatedScenarios);
    setIsCreating(false);
    setActiveScenario(null);
    
    // ✅ FIX: Pass updated array directly
  ...
  // ✅ FIX: Pass updated scenarios directly
  ...
  const duplicateScenario = (scenario) => {
    ...
    const updatedScenarios = [...scenarios, duplicate];
    setScenarios(updatedScenarios);
    saveScenarios(updatedScenarios);
  };
