...
const KnowledgeBaseTab = ({ botConfig, updateConfig, Upload, Globe, Link, BookOpen, Trash }) => {
  ...
  const discoverSubpages = async (url) => {
    ...
    try {
      ...
      debugLog('Valid URL detected', { domain, baseUrl });

      // Since API is unavailable, simulate common page discovery
      // Generate realistic subpages based on domain
      const commonPages = [
        { path: '/', title: 'Home Page' },
        { path: '/about', title: 'About Us' },
        { path: '/about/team', title: 'Our Team' },
        { path: '/about/mission', title: 'Mission & Values' },
        { path: '/products', title: 'Products' },
        { path: '/products/category/featured', title: 'Featured Products' },
      ...
      // Create full URLs
      const discoveredPages = commonPages.map(page => ({
        url: `${baseUrl}${page.path}`,
        title: page.title,
        selected: page.path === '/' // Auto-select homepage
      }));

      // Add a delay to simulate discovery
      await new Promise(resolve => setTimeout(resolve, 1000));

      debugLog('Discovered pages', { count: discoveredPages.length });
      setDiscoveredPages(discoveredPages);
      setSelectedPages(discoveredPages.filter(p => p.selected).map(p => p.url));
      setShowSubpages(true);
      
    } catch (error) {
      debugLog('Discovery error', error);
      console.error('Error discovering subpages:', error);
      alert(`Invalid URL format. Please enter a valid website URL (e.g., truecitrus.com or https://truecitrus.com)`);
    } finally {
  ...
  // Scrape selected pages (Demo Mode - Since API is unavailable)
  const scrapeSelectedPages = async () => {
    ...
    debugLog('Scraping selected pages (Demo Mode)', { count: selectedPages.length });
    setWebScraping(true);
    ...
    for (let i = 0; i < selectedPages.length; i++) {
      ...
      try {
        ...
        // Since API is unavailable, generate simulated content
        const urlObj = new URL(pageUrl);
        const pagePath = urlObj.pathname || '/';
        const pageTitle = discoveredPages.find(p => p.url === pageUrl)?.title || 'Page';
        
        // Generate simulated content based on page type
        const simulatedContent = `${pageTitle}\n\nThis is simulated content for ${pageUrl}.\n\nKey information:\n- Product features and benefits\n- Company information and services\n- Contact details and support options\n- Pricing and availability\n\nNote: This is demo content. In production, real web scraping would extract actual page content including text, headings, metadata, and structured data from the website.`;
        ...
        const knowledgeItem = {
          ...
          name: pageTitle,
          type: 'webpage',
          content: simulatedContent.substring(0, 300) + '...',
          fullContent: simulatedContent,
          status: 'processed',
          uploadDate: new Date().toISOString().split('T')[0],
          wordCount: simulatedContent.split(/\s+/).filter(word => word.length > 0).length,
          url: pageUrl,
          source: 'web'
        };
      ...
      // Small delay between requests to simulate scraping
    ...
    if (scrapedItems.length > 0) {
      ...
      alert(`✅ Successfully added ${scrapedItems.length} page(s) to knowledge base!\n\n⚠️ Demo Mode: Using simulated content. Connect API for real web scraping.`);
    }
